...
namespace ClubManagement.Games.Service
{
    public class RecordBoardService
    {
        ...
        /// <returns></returns>
        private List<GameOrderDto> LoadGames(int match)
        {
        ...
        /// <param name="match"></param>
        /// <returns></returns>
        private List<PlayerInfoDto> LoadAttendees(int match)
        {
        ...
        /// 사이드 게임 플레이어 리스트 생성
        /// </summary>
        private void SetSideGamePlayer(RecordBoardModel model)
        {
            model.SideGamePlayers = model.PlayerList.Where(p => p.IndividualSide).ToList();
            model.AllcoverGamePlayers = model.PlayerList.Where(p => p.AllCoverSide).ToList();
        }
